...
import { QueueRecallBodySchema } from "@/lib/schemas/admin";
import { recallPatient } from "@/lib/services/queue.service";
...
export const POST = withStaffAuth(async (request: Request, context?: { params?: Record<string, string> }) => {
  ...
  const parsed = await parseJsonBody(request, QueueRecallBodySchema);
